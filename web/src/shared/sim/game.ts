...
export class GameSimulation {
  ...
  private readonly outgoingDamageScales = new Map<PlayerId, number>();
  ...
  removePlayer(id: PlayerId): void {
    ...
    this.outgoingDamageScales.delete(id);
  ...
  setOutgoingDamageScale(id: PlayerId, scale: number): void {
    this.outgoingDamageScales.set(id, clamp(scale, 0, 1));
  }
  ...
  damagePlayer(victim: PlayerState, info: DamageInfo): { applied: number; killed: boolean } {
    ...
    if (info.attacker !== victim.id) {
      info.amount *= this.outgoingDamageScales.get(info.attacker) ?? 1;
    }
