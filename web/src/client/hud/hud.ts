...
export class Hud {
  ...
  private renderScoreboard(world: WorldState, localId: PlayerId): void {
    ...
    if (teamBased) {
      const localTeam = world.players.get(localId)?.team ?? Team.Allies;
      const friendlies = players.filter((p) => !isEnemyTeam(localTeam, p.team));
      const enemies = players.filter((p) => isEnemyTeam(localTeam, p.team));
      this.els.scoreboard.innerHTML =
        `<div class="sb-team sb-friendly"><h3>我方</h3><table>${header}${friendlies.map(row).join('')}</table></div>` +
        `<div class="sb-team sb-enemy"><h3>敵方</h3><table>${header}${enemies.map(row).join('')}</table></div>`;
    } else {
      this.els.scoreboard.innerHTML =
